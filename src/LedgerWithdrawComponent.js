...
class LedgerWithdrawComponent extends Component {
  constructor(props) {
    super(props);
  ...
  generateTx = async () => {
    const txMsg = {
        msg: [{"type":"cosmos-sdk/MsgWithdrawDelegationReward","value":{"delegator_address":this.state.address,"validator_address":this.props.validator_addr}}],
    ...
    try {
      ...
      if(this.state.version === 36864 && this.state.address == null) {
        ...
        try {
          ...
          const rewardsInfo = await lcd.getRewardsInfo(this.props.lcd, address, this.props.validator_addr)
      ...
      {this.state.addressOpen &&
        ...
        <div>
          <div className="padd-bot-25">
            ...
            <button className="btn btn-auth-login" onClick={() => this.generateTx()}>Generate & Sign Withdraw Transaction</button>
          </div>
        </div>
        }
        {
...
export default LedgerWithdrawComponent;
